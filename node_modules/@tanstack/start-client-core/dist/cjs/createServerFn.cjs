...
setServerFnStaticCache(() => {
  ...
  return createServerFnStaticCache({
    ...
    setItem: async (ctx, response) => {
      ...
      await fs.writeFile(filePath, client.tsrSerializer.stringify(response));
    },
    fetchItem: async (ctx) => {
      const hash = jsonToFilenameSafeString(ctx.data);
      const url = await getStaticCacheUrl(ctx, hash);
      ...
      if (!result) {
        result = await fetch(url, {
          method: "GET"
        }).then((r) => r.text()).then((d) => client.tsrSerializer.parse(d));
...
function extractFormDataContext(formData) {
  ...
  try {
    const context = client.tsrSerializer.parse(serializedContext);
    return {
      context,
      data: formData
    };
